...
|operation
|Operation to perform on the API instance. Use `create` to create a new API instance or `configureBasicEndpointInMuleApplication` to configure a basic endpoint in a Mule application.
|Yes
...
|operation
|Operation to perform on the asset. Use `create` to create a new asset, `update` to modify an existing asset, or `delete` to remove an asset.
|Yes
...
|dataFormat
|The format of the metadata being processed. Specify `JSON` for JSON-formatted metadata or `XML` for XML-formatted metadata.
|No
...
|specLanguage
|Specification language for the API project. Use `RAML` for RESTful API Modeling Language or `OAS` for OpenAPI Specification.
|Yes
...
Prerequisite: Install the xref:mcp-connector::index.adoc[MCP Connector].
...
|target
|Deployment target environment. Use `CloudHub-US` for CloudHub US region, `CloudHub-EU` for CloudHub EU region, or `RuntimeFabric` for Runtime Fabric deployment.
|Yes
...
|language
|Language of the API specification to generate. Use `oas` for OpenAPI specifications or `raml` for RAML specifications.
|Yes
...
* `control_flow` to stop request flow and generate a custom response.
* `cors` to use the Cross-Origin Resource Sharing library.
* `dataweave` to define and evaluate custom DataWeave expressions.
* `grpc` to make gRPC call to external service.
* `header_manipulation` to perform CRUD for request/response headers.
* `http_call` to make HTTP call to external service.
* `jwt` to parse and validate JSON Web Token.
...
|latencyPercentile
|If `includeLatency=true`, this parameter specifies which percentile to use for latency measurements. Use `P99` for 99th percentile, `P95` for 95th percentile, `P90` for 90th percentile, or `P75` for 75th percentile. Default is `P99`.
|No
...
|operation
|Operation to perform on the Flex Gateway policy project. Use `create` to create a new project, `configure` to set up project configuration, `build` to compile the project, `publish-dev` to publish to development environment, or `publish-release` to publish a release version.
|Yes
