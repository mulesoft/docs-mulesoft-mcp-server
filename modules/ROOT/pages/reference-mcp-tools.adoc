...
* `deploy_agent_network`: Deploy an agent network instance to a runtime environment. Specify the project path, target environment, private space, and Managed Flex Gateway configuration for deployment. Prerequisites: Managed Flex Gateway, a business group, and the agent network project contains a valid `exchange.json` file. Available only in Anypoint Code Builder.
...
* `open_api_spec`: Open an API specification project. Available only in Anypoint Code Builder.
