...
|`javaHome`
|Get JAVA_HOME from JDK from \"AnypointCodeBuilder\" (grep for exact name) installation if exists. Installation is usually located in user home directory. If installation does not exist, get user's java_home. If multiple JDKs exist, choose the latest one that matches javaSpecificationVersions in mule-artifacts.json. If JDK version does not match any of the versions in mule-artifacts.json, return 'No JAVA_HOME found. Don't retry.'.
|Yes
...
Create a new Mule project without flow generation. This tool focuses on just the project structure setup with optional parameters.
...
|`projectPath`
|Directory to create project in. Default is the current workspace.
|No
