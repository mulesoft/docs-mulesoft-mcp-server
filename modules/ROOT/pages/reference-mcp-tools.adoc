...
|<<uc-publish-connector,uc_publish_connector>>
|No
|Yes
|None
...
|<<get-flex-gateway-policy-example,get_omni_gateway_policy_example>>
|No
|No
|None
...
|<<manage-flex-gateway-policy-project,manage_omni_gateway_policy_project>>
|No
|No
|None
...
Add pagination support to a specific CCM operation. Provide the operation file path and pagination configuration so the tool can create a paginated operation module.
...
|`operationFilePath`
|URI of the operation file. Must be a valid path to an operation file in the CCM operations directory.
|Yes

|`type`
|Pagination type to apply. Valid values:
...
|`itemsField`
|Path to the array of items in the response (for example, `data.items` or `results`). Specifies where paginated items appear in the API response.
|No

|`offsetParam`
|Name of the offset parameter in the API request (for example, `offset` or `skip`).
|Yes, when `type` is `offset`.

|`initialOffsetValue`
|Initial value for the offset parameter (typically `0`).
|Yes, when `type` is `offset`.

|`pageNumberParam`
|Name of the page number parameter in the API request (for example, `page` or `pageNumber`).
|Yes, when `type` is `page`.

|`initialPageNumber`
|Initial page number (typically `0` or `1`, depending on the API).
|Yes, when `type` is `page`.

|`timestampParam`
|Name of the timestamp parameter in the API request (for example, `since` or `after`).
|Yes, when `type` is `timestamp`.

|`cursorParam`
|Name of the cursor parameter in the API request (for example, `cursor` or `nextToken`).
|Yes, when `type` is `cursor`.

|`referenceType`
|Reference type for cursor pagination. Use `marker` for a simple marker or token value, or `uri` for a URI or URL that contains the next page reference.
|Yes, when `type` is `cursor`.

|`referenceExpression`
|Expression that extracts the next cursor value from the API response (for example, `pagination.nextCursor`). Use a path expression that points to the cursor value in the response payload.
|Yes, when `type` is `cursor`.
...
----
{
  ...
  "arguments": {
    "operationFilePath": "file:///Users/developer/my-connector/src/main/resources/ccm/operations/listItems.json",
    "type": "offset",
...
{
  "success": true,
  "message": "Pagination module created for operation listItems.",
  "operationFilePath": "/Users/developer/my-connector/src/main/resources/ccm/operations/listItems.json",
...
List available connection profiles with their IDs, names, authentication types, and descriptions. Use this tool to see which profiles you can use when executing connector operations. The profile ID returned here is required for the `execute` type in <<uc-manage-operations,`uc_manage_operations`>>.
...
----
{
  ...
  "profiles": [
    {
      "id": "profile-sandbox-01",
      "name": "Sandbox API Key",
      "authType": "apiKey",
      "description": "Sandbox environment with API key authentication"
    }
  ]
}
----
...
Manage connector operations: list operations, get details (including input and output schemas), or execute an operation using a connection profile. To execute an operation, first call <<uc-list-connection-profiles,`uc_list_connection_profiles`>> to obtain a profile ID, then call this tool with `type` set to `execute`, the operation name, the profile ID, and an `input` object containing the operation parameters.
...
* `get` to get details for one operation (including schemas)
* `execute` to run an operation with a connection profile
|Yes

|`operationName`
|Name or ID of the operation (for example, from a prior `list` or `get` call).
|Yes, when `type` is `get` or `execute`.

|`profileId`
|Connection profile ID from `uc_list_connection_profiles` to use for authentication.
|Yes, when `type` is `execute`.

|`input`
|When `type` is `execute`, operation parameters as a JSON object. Use `type`=`get` with the operation name first to see the required or optional input fields.
|No

|===

.Example request (list operations)
[source,json]
----
{
...
.Example request (execute operation)
[source,json]
----
{
  "name": "uc_manage_operations",
...
[[uc-publish-connector]]
== uc_publish_connector

Publish the connector to Anypoint Exchange. The tool chooses execution mode from your configuration: if `organizationId` or `repositoryId` is missing and not found in the project POM, it runs in workflow mode and returns a plan to collect configuration; if the required configuration is present, it runs in granular mode and performs the publish.
...
|`workflowCustomization`
|Optional preferences or requirements for the publish workflow.
|No

|`organizationId`
|Anypoint Exchange organization ID. If omitted, the tool reads the POM. If the ID is still missing, workflow mode is used.
|No

|`repositoryId`
|Private Exchange repository ID. If omitted, the tool reads the POM. If the ID is still missing, workflow mode is used.
|No

|`profiles`
|Maven profiles to use for deployment. Defaults to `private-exchange-deploy` if not specified.
|No
...
----
{
  "name": "uc_publish_connector",
  "arguments": {
    "organizationId": "a1b2c3d4-e5f6-7890-abcd-ef1234567890",
    "repositoryId": "my-private-exchange-repo"
  }
}
----

.Example response (granular mode)
[source,json]
----
{
  "success": true,
