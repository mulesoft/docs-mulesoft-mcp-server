...
.16+s|Application Development
|<<create-and-manage-assets,create_and_manage_assets>>
|No
|No
a|
* Exchange: Exchange Administrator (`manage:exchange`)
* Exchange: Exchange Contributor (`edit:exchange`)
* Exchange: Exchange Creator (`create:exchange`)
...
|No
|No
|Anypoint Code Builder: Mule Developer Generative AI User (`create:generations`)
...
|<<generate-or-modify-munit-test,generate_or_modify_munit_test>>
|Yes
|No
|Anypoint Code Builder: Mule Developer Generative AI User (`create:generations`)
...
|Yes
|No
|Exchange: Exchange Viewer (`read:exchange`)
...
.9+s|Connector Development
|<<uc-add-pagination,uc_add_pagination>>
|No
|Yes
|None

|<<uc-connector-lifecycle,uc_connector_lifecycle>>
|No
|Yes
|None
...
|<<uc-generate-tests,uc_generate_tests>>
|No
|Yes
|None

|<<uc-get-test-generation-info,uc_get_test_generation_info>>
|No
|Yes
|None

|<<uc-list-connection-profiles,uc_list_connection_profiles>>
|No
|Yes
|None

|<<uc-manage-api-spec,uc_manage_api_spec>>
|No
|Yes
|None

|<<uc-manage-operations,uc_manage_operations>>
|No
|Yes
|None
...
|No
|No
|API Manager: Manage API Configuration (`manage:api_configuration`)
...
a|
* Exchange: Exchange Viewer (`read:exchange`)
* Runtime Manager: View Connected Applications (`read:orgconnapps`)
* Runtime Manager: Create Applications (`create:applications`)
* Runtime Manager: Read Runtime Fabrics (`read:runtime_fabrics`)

|<<list-api-instances,list_api_instances>>
|No
|No
|API Manager: View APIs Configuration (`read:api_configuration`)

|<<list-applications,list_applications>>
|No
|No
a| 
* Runtime Manager: Monitoring Viewer (`view:monitoring`)
* Runtime Manager: View Connected Applications (`read:orgconnapps`)
* Runtime Manager: Read Applications (`read:applications`)
* Runtime Manager: Read Runtime Fabrics (`read:runtime_fabrics`)
...
|No
a|
* General: View Organization (`read:organization`)
* Runtime Manager: View Connected Applications (`read:orgconnapps`)
* Runtime Manager: Create Applications (`create:applications`)
...
|No
|No
|API Manager: Manage Policies (`manage:api_policies`)
...
* Runtime Manager: Monitoring Viewer (`view:monitoring`)
* Runtime Manager: Read Runtime Fabrics (`read:runtime_fabrics`)
* Runtime Manager: CloudHub Network Viewer (`read:cloudhub_networking`)
* General: Usage Viewer (`view:metering`)
* General: View Organization (`read:organization`)
* API Manager: View APIs Configuration (`read:api_configuration`)
...
Provide instructions to the LLM on how to manage an Omni Gateway custom policy project. Instructions include how to create the project, set up the configuration, build the project, publish a development version of the project, and publish a release version of the project. 
...
|`operation`
|Operation to perform on the Omni Gateway policy project. Use `create` to create a project, `configure` to set up project configuration, `build` to compile the project, `publish-dev` to publish to development environment, or `publish-release` to publish a release version.
|Yes

|`projectPath`
|Path to the Omni Gateway policy project directory.
|No
...
|`exchangeScope`
|Determines the scope of an asset search in Exchange. Set the appropriate scope:

* For MuleSoft provided assets: `exchangeScope: public`
* For private assets (default): `exchangeScope: private`
...
Deploy an agent network instance to a runtime environment. Specify the project path, target environment, deployment target, and Managed Omni Gateway configuration for deployment.
...
[[generate-or-modify-munit-test]]
== generate_or_modify_munit_test

Generate, modify, or fix MUnit tests for Mule flows. Provide the flow XML, test scenario description, and dependencies for accurate test generation.

[%header,cols="1,3a,1"]
|===
|Parameter |Description |Required?

|`projectSource` | The Mule flow definition, in XML format, for which an MUnit test needs to be generated. Do not include the namespace declaration. Only send the complete flow definition. Required for generating, modifying or fixing an MUnit test. |Yes
|`munitFileName` | The file name of the MUnit test. Generate the file name from the source file name in the format `<mule-configuration-file-name>-test-suite.xml`. |Yes for generating an MUnit test
|`prompt` |A prompt that explains the validation of the test case, for example, "Verify the flow returns a 400 error when an invalid payload is received." |Yes for generating, modifying or fixing an MUnit test
|`minRuntimeVersion` |The minimum runtime version required for the flow. |Yes for generating an MUnit test
...
|`existingMunit` |The XML source of the existing MUnit test case that needs to be modified, including the `<munit:test>` element and all nested components, such as mocks, variables, and assertions. |Yes for modifying or fixing an MUnit test.
...
* `ip_filter` to allow or block requests based on IP addresses and CIDR ranges.
* `jwt` to parse and validate JSON Web Tokens.
* `jwt_generate` to generate JSON Web Tokens.
* `json_validator` to validate JSON payloads.
* `lock` to synchronize workers.
* `logger` to print custom messages.
* `metadata` to access metadata of the policy, API, and environment.
* `oauth2_token_introspection` to validate OAuth2 tokens.
* `outbound` to define an outbound policy.
* `policy_violation` to mark requests as violations in monitoring traffic.
* `rate_limiting` to allow or block requests based on rate limits.
* `request_data` to share data between requests and responses.
* `spike_control` to limit requests by queuing or rejecting traffic that exceeds a sliding-window rate limit.
* `stop_iteration` to enable processing of headers and body in the same request/response state.
* `timer` to perform sleep operations.
* `unit_testing` to generate unit tests for your custom policy.
* `worker_variable` to share data across requests of a single worker.
* `xml_validator` to validate XML payloads.
|Yes
...
Add pagination support to a specific CCM operation. Takes the operation file path and pagination configuration parameters and creates a paginated operation module.
...
|`operationFilePath`
|Path (URI) to an operation file inside the CCM operations directory.
|Yes

|`type`
|Pagination strategy to apply. Valid values are:
...
|`itemsField`
|Path to the array of items in the response, for example, `data.items` or `results`.
|No

|`offsetParam`
|Name of the offset parameter, for example, `offset` or `skip`.
|Required when `type='offset'`.

|`initialOffsetValue`
|Starting offset value (typically `0`).
|Required when `type='offset'`.

|`pageNumberParam`
|Name of the page number parameter, for example, `page` or `pageNumber`.
|Required when `type='page'`.
...
|`timestampParam`
|Name of the timestamp parameter, for example, `since` or `after`.
|Required when `type='timestamp'`.

|`cursorParam`
|Name of the cursor parameter, for example, `cursor` or `nextToken`.
|Required when `type='cursor'`.

|`referenceType`
|Type of reference used for cursor pagination. Valid values are:

* `marker` for a simple marker or token value
* `uri` for a URI containing the next page reference

|Required when `type='cursor'`.

|`referenceExpression`
|Path expression to extract the next cursor value from the response, for example, `pagination.nextCursor`.
|Required when `type='cursor'`.

|`projectRootPath`
|Absolute path to the connector project directory. Defaults to the workspace root folder.
|No
...
----
{
  "name": "uc_add_pagination",
  "arguments": {
    "operationFilePath": "/Users/you/workspace/stripe-connector/operations/list-customers.yaml",
    "type": "cursor",
    "cursorParam": "starting_after",
    "referenceType": "marker",
    "referenceExpression": "data[-1].id",
    "itemsField": "data"
  }
}
----
...
{
  "success": true,
  "message": "Pagination added to operation."
}
----

[[uc-connector-lifecycle]]
== uc_connector_lifecycle

Manage connector lifecycle operations. Use `type='build'` to build the connector project using the extension's bundled Maven (`mvn install`). Use `type='publish'` to publish the connector to Anypoint Exchange (`mvn deploy`). For publish, the tool automatically determines execution mode: granular mode if all required configuration is available (executes publish directly), or workflow mode if `organizationId` or `repositoryId` is missing (returns a workflow plan to collect configuration).

Available only in Connector Builder.

NOTE: This tool was previously named `uc_publish_connector`. The renamed tool also supports the `build` action.
...
|`type`
|Lifecycle action to perform. Valid values are:

* `build` to compile and package the connector project
* `publish` to deploy the connector to Anypoint Exchange

|Yes

|`cleanBeforeBuild`
|Whether to run `clean` before `install`. Used when `type='build'`. Defaults to the corresponding VS Code setting.
|No

|`workflowCustomization`
|Specific workflow customizations, preferences, or requirements for the publish process. Used when `type='publish'`.
|No

|`organizationId`
|Anypoint Exchange organization ID. If not provided, the tool checks the POM file. If missing there too, workflow mode is triggered. Used when `type='publish'`.
|No

|`repositoryId`
|Private Exchange repository ID. Same fallback behavior as `organizationId`. Used when `type='publish'`.
|No

|`profiles`
|Maven profiles to activate for deployment. Defaults to `['private-exchange-deploy']`. Used when `type='publish'`.
|No
...
{
  "success": true,
  "message": "Connector published to Anypoint Exchange."
}
----
...
|`mode`
|Execution mode. Valid values are:

* `standard` (default) runs interactively with user confirmation steps.
* `connectorless` runs autonomously and only pauses to collect credentials.

|No

|`platform`
|Name of the SaaS platform, for example, `Stripe` or `ServiceNow`. Pre-populates the workflow when provided.
|No

|`useCases`
|List of integration use cases, for example, `Sync customer data`. Pre-populates the workflow when provided.
|No

|`documentation`
|List of API documentation URLs. Pre-populates the workflow when provided.
|No
...
----
{
  ...
  "nextSteps": "Begin with step 1 by providing the platform name, use cases, and documentation URLs to the uc_manage_api_spec tool.",
  "customizations": [
...
Generate a complete LinkWeave connector from an API spec. Supports flexible multi-phase generation including scaffolding, CCM (Canonical Connectivity Model), Anypoint Connector, and Flow Connector. All endpoints from the API spec are used. Can execute phases individually or in combination.
...
|`phases`
|Array of phases to execute - any combination of `ccm`, `anypoint`, and `flow`. Phases execute in order. Valid values are:

* `ccm` for Canonical Connectivity Model
* `anypoint` for Anypoint Connector module
* `flow` for Flow Connector module

|No

|`projectRootPath`
|Absolute path to the directory where the connector project should be created (for the `ccm` phase) or found (for the `anypoint` or `flow` phases). Defaults to the workspace root folder.
|No
...
Generate tests for connector modules. Each requested module is configured to use either a mock connection (no auth) or a real connection (requires a `profileId`). At least one module must be included. Always call `uc_get_test_generation_info` first to determine which modules are available and whether tests already exist.
...
|`modulesConfig`
|Array of modules to test. Skipped modules are omitted from this array. At least one module is required.
|Yes

|`modulesConfig[].moduleType`
|Module type to test. Valid values are:

* `CCM` for the connectivity model
* `ACM` for the Anypoint Connector module
* `FCM` for the Flow Connector module

|Yes

|`modulesConfig[].useMock`
|Whether to use a mock connection. Set to `true` for a mock connection (no auth required) or `false` for a real backend connection (requires `profileId`).
|Yes

|`profileId`
|Connection profile ID returned by `uc_list_connection_profiles`.
|Required when any module has `useMock=false`.

|`forceOverwrite`
|Whether to overwrite existing test files. Set to `true` only after the user explicitly confirms overwriting. Defaults to `false`.
|Required when existing tests are present.
...
----
{
  "name": "uc_generate_tests",
  "arguments": {
    "modulesConfig": [
      { "moduleType": "CCM", "useMock": true },
      { "moduleType": "ACM", "useMock": false }
    ],
    "profileId": "dev-stripe",
...
{
  "success": true,
  "message": "Tests generated for CCM and ACM modules."
}
----

[[uc-get-test-generation-info]]
== uc_get_test_generation_info

Get information before generating tests. Returns the list of available connector modules (CCM, ACM, FCM) and reports whether tests already exist for each. Always call this tool before `uc_generate_tests` so you can determine which modules to test and whether existing test files would be overwritten.
...
----
{
  "name": "uc_get_test_generation_info",
  "arguments": {}
}
----
...
----
{
  "availableModules": ["CCM", "ACM", "FCM"],
  "testsExist": false
}
----

[[uc-list-connection-profiles]]
== uc_list_connection_profiles

List all available connection profiles with their IDs, names, authentication types, and descriptions. Use this tool to discover which profiles are available for executing operations or generating tests against a real backend. The profile ID returned by this tool is required for the `execute` type in `uc_manage_operations` and for real-connection test generation in `uc_generate_tests`.
...
----
{
  "name": "uc_list_connection_profiles",
  "arguments": {}
}
----
  ...
  "profiles": [
    {
      ...
      "description": "Stripe API developer credentials"
    }
  ]
}
----
...
Manage API spec operations. Use `type='generate'` (default) to perform API research and produce both an `api-reference.md` artifact and a complete OpenAPI 3.0 YAML spec. Use `type='research'` to perform API research only, producing a comprehensive `api-reference.md` with entity dependency graphs, CRUD endpoint catalogs, and field-level detail. Use `type='validate'` to validate an existing OpenAPI spec against a live API by executing real HTTP requests, comparing responses against expected schemas, auto-fixing mismatches, and generating a validation report. If no parameters are used, the agent prompts for input.

Available only in Connector Builder.

NOTE: This tool was previously named `uc_generate_api_spec`. The renamed tool also supports the `research` and `validate` modes.
...
* `generate` (default): Performs API research and generates an OpenAPI YAML spec.
* `research`: Performs API research only and produces an `api-reference.md` artifact.
* `validate`: Validates an OpenAPI spec against a live API and generates a validation report.
...
* `standard` (default): Runs interactively.
* `connectorless`: Runs autonomously with minimal user interaction.

|No

|`specPath`
|Path to the OpenAPI YAML spec file to validate. Used when `type='validate'`.
|No

|`apiReferencePath`
|Path to the `api-reference.md` file containing entity details, dependency graphs, and request/response schemas. Used when `type='validate'`.
|No

|`baseUrl`
|API base URL override. If not provided, extracted from the OpenAPI spec or `api-reference.md`. Used when `type='validate'`.
|No

|`authHeader`
|Authorization header value, for example, `Bearer <token>`. If not provided, the tool prompts for credentials. Used when `type='validate'`.
|No

|`operationFilter`
|List of specific operation IDs to validate. If empty, validates all operations. Used when `type='validate'`.
|No
...
{
  "success": true,
  "message": "Generated api-reference.md and OpenAPI 3.0 spec from documentation and use cases.",
  "platform": "Stripe",
  "apiReferencePath": "/Users/you/workspace/stripe-spec/api-reference.md",
  "openApiYamlPath": "/Users/you/workspace/stripe-spec/openapi.yaml"
}
----

[[uc-manage-operations]]
== uc_manage_operations

Manage connector operations. Use `type='list'` to enumerate available operations, `type='get'` to retrieve one operation's input and output schemas, or `type='execute'` to run an operation against a connection profile. To execute an operation, first call `uc_list_connection_profiles` to retrieve a profile ID.
...
* `list` to list all operations
* `get` to get details for one operation
* `execute` to run an operation with a connection profile

|Yes

|`operationName`
|Operation name or ID.
|Required when `type='get'` or `type='execute'`.
...
|`input`
|Operation input parameters as a JSON object. Use `type='get'` first to discover the expected schema.
|No
