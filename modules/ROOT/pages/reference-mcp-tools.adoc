...
|<<dataweave-create-documentation,dataweave-create-documentation>>
|No

|<<dataweave-create-project,dataweave-create-project>>
|No

|<<dataweave-create-sample-data,dataweave-create-sample-data>>
|No

|<<dataweave-get-module-metadata,dataweave-get-module-metadata>>
|No

|<<dataweave-get-project-metadata,dataweave-get-project-metadata>>
|No

|<<dataweave-open-project,dataweave-open-project>>
|No

|<<dataweave-run-script-tool,dataweave-run-script-tool>>
|No
...
|organizationId
|The ID of the org to publish the org to.
|No

Optional for asset creation.

|classifier
|The type of asset. Valid values are `raml-fragment`, `raml`, `oas`, `wsdl`, `http-api`, `custom`, `evented-api`, `graphql`, `mule-application`, and `mule-plugin`.
...
|projectPath
|Path of the Mule project directory where the pom.xml is located.
|Yes
...
Creates a new DataWeave project, returns the URL of the created project created, and prompts to open the project.
...
Prerequisite: Create the DataWeave script file before running `dataweave-create-sample-data`.
...
|url
|The URL of DataWeave transformation file to create sample input for. The file must be in the format of `file:://path`. For example, `\file:///home/mdeachaval/test.dwl`. By default, uses the currently open file.
|Yes
...
|moduleUrl
|The URL of the DataWeave module file in the format `file://<path>.dwl` or `jar://<path>.dwl` search documentation for. Use the `dataweave-get-project-metadata` tool to get the URL of a given module.
|Yes
...
Runs a DataWeave transformation using the sample data as input context and displays the output. Use to validate a script output. If the tool fails with missing input, create sample data using `dataweave-create-sample-data` tool.
...
|Enables performance optimization features.
|No
...
Prerequisites: Log in to Anypoint Code Builder with your Anypoint Platform credentials and enable Einstein in Access Management.
...
|timeFrame
|If `includeCpuMetrics`, `includeHeapMetrics`, or `includeLogs` is true, this parameter contains the time frame for metrics. Valid values are `5m`, `15m`, `30m`, `1h`, `3h`, `6h`, `12h`, `24h`, `3d`, `5d`, `7d`, `15d`, and `30d`. The default value is `30m`.
|No
...
|sortCriteria
|Sort search results. Valid values are `name` or `last_modified_date`. Defaults to `last_modified_date`.
|No
