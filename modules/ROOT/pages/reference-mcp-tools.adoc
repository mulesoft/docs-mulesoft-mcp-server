...
|version
|Version of the asset. Example: 1.4.0.
|Yes
...
|mode
|The operation to perform on the agent network project YAML. Valid values are 'configure' and 'edit'. For example, use 'configure' to set up a custom agent network tailored to your environment, discover agent network components, or scaffold a new agent network. Use 'edit' to make minor changes to a configured agent network YAML file, such as renaming an existing agent.
|Yes
...
|operation
|Operation to perform. Valid values: `create`, `update`, `delete`.
|Yes
...
|operation
|Task to perform. Valid values: 'define' to create metadata, 'attach' to apply metadata to flow components, and 'update' to update existing metadata.
|Yes

|metadata
|String value of sample data or schema JSON object. JSON original format is required (no conversion from other formats). Required when the operation parameter value is 'define'.
|No
...
|metadataPath
|Path of the sample metadata or schema file. Required when the operation parameter value is 'define'.
|No

|componentNodeDocId
|The doc:id attribute value for the flow component node. Don't provide the child node's doc:id. Required when the operation parameter value is 'attach'.
|No

|metadataName
|Name of the new metadata to be defined based on a sample data JSON. Example: 'customer' if sample data is for a customer. Required when the operation parameter value is 'define'.
|No

|isSchema
|Whether the metadata is a schema. Required when the operation parameter value is 'define'.
|No
...
|specLanguage
|Specification language. Valid values: `RAML`, `OAS`.
|Yes
...
|target
|Deployment target. Valid values: `CloudHub-US`, `CloudHub-EU`, `RuntimeFabric`.
|Yes
...
|operation
|Operation to perform. Valid values: `create`, `configure`, `build`, `publish-dev`, `publish-release`.
|Yes
