...
These tools are available in MuleSoft MCP Server. Some tools are available only in the MuleSoft MCP Server that's included in Anypoint Code Builder IDE; those tool descriptions include "Available only in Anypoint Code Builder."
...
* `validate_api_against_rules`: Validate an API specification in your project against governance rulesets or rules that you specify. If you specify all rulesets, the tool uses an empty 'rules' parameter to validate all rulesets. Similarly, if you specify no ruleset, the tool uses an empty `rules` parameter. To find rules or rulesets that you specify, this tool calls the `list_governance_rulesets` tool. Available only in Anypoint Code Builder.
