...
Requires the `CONTEXT_CATALOG_ENABLED` flag to be enable in your organization.
...
Get hierarchy, classifications, and PII state for a single data catalog source. Before using this tool, use `find_assets` with `asset_type=data` to discover available sources. Use this tool to answer questions such as "What's the PII classification for the customer_orders data source?" or "Is there any PII in the finance_reporting data catalog source?"
