...
|read:api_configuration, read:api_policies
...
.9+s|Cost Management
|<<create-model-wallet,create_model_wallet>>
a|
...
|<<delete-model-wallet,delete_model_wallet>>
a|
* API Manager: Manage Policies (`manage:api_policies`)
* API Manager: View APIs Configuration (`read:api_configuration`)
* API Manager: View Policies (`read:api_policies`)
...
|<<edit-model-wallet,edit_model_wallet>>
a|
* API Manager: Manage Policies (`manage:api_policies`)
* API Manager: View APIs Configuration (`read:api_configuration`)
* API Manager: View Policies (`read:api_policies`)
...
|<<fetch-model-wallet-details,fetch_model_wallet_details>>
a|
* API Manager: Manage Policies (`manage:api_policies`)
* API Manager: View APIs Configuration (`read:api_configuration`)
* API Manager: View Policies (`read:api_policies`)
...
|<<fetch-model-wallets,fetch_model_wallets>>
a|
* API Manager: Manage Policies (`manage:api_policies`)
* API Manager: View APIs Configuration (`read:api_configuration`)
* API Manager: View Policies (`read:api_policies`)
...
|<<describe-vault-auth-schema,describe_vault_auth_schema>>
|Manage vault integrations

|<<edit-integration,edit_integration>>
|Manage vault integrations

|<<list-integration-secrets,list_integration_secrets>>
|Manage vault integrations, read secrets metadata, and manage secret groups

|<<onboard-vault,onboard_vault>>
|Manage vault integrations

|<<search-secrets,search_secrets>>
|Manage vault integrations, read secrets metadata, and manage secret groups

|<<test-integration-connection,test_integration_connection>>
|Manage vault integrations
...
|`environment_id`
|Target Anypoint environment ID. Required unless `environment_name` is provided.
|No

|`environment_name`
|Human-readable environment name. Provide this *or* `environment_id`, not both.
|No

|`secret_group_name`
|Human-readable name of the secret group (as returned by `list_secret_refs` in `secretGroupName`). Pair with `secret_name`.
|No

|`secret_name`
|Human-readable name of the secret. Pair with `secret_group_name`.
|No

|`secret_group_id`
|ID of the secret group. Pair with `secret_id`.
|No

|`secret_id`
|ID of the secret. Pair with `secret_group_id`.
|No

|===

NOTE: You must provide either (`secret_group_name` and`secret_name`) or (`secret_group_id` and `secret_id`), not both. You must also provide either `environment_id` or `environment_name`.
...
|`cadence`
|Budget replenishment period (for example, `weekly`, `monthly`).
|Yes
...
|`provider`
|Vault provider to describe (`aws_secrets_manager`, `azure_key_vault`, `hashicorp_vault`). Omit along with `auth_method` to list all supported combinations.
|No

|`auth_method`
|Authentication method to describe (for example, `azure_sp_secret`). Must be paired with `provider`; omit both to list all combinations.
|No
...
.Example request (specific combination)
[source,json]
----
{
  "name": "describe_vault_auth_schema",
...
|`environment_id`
|Target environment ID. Required unless `environment_name` is provided.
|No
...
|`integration_name`
|Human-readable name of the vault integration. Provide this or `integration_id`, not both.
|No

|`integration_id`
|UUID of the vault integration. Provide this or `integration_name`, not both.
|No
...
NOTE: You must provide either `integration_name` or `integration_id`. You must also provide either `environment_id` or `environment_name`.
...
|`provider`
|Vault provider. One of: `aws_secrets_manager`, `azure_key_vault`, `hashicorp_vault`.
|Yes

|`auth_method`
|Authentication method for the provider (for example, `aws_static`, `azure_sp_secret`).
|Yes
...
|How often (minutes) to re-sync. Must be between `120` and `1440`. Default: `120`.
|No
...
|`environment_id`
|Target Anypoint environment UUID. Required unless `environment_name` is provided.
|No
...
|`integration_name`
|Post-create mode: human-readable vault integration name. Provide this or `integration_id`.
|No

|`integration_id`
|Post-create mode: vault integration UUID. Provide this or `integration_name`.
|No

|`provider`
|Pre-create mode: vault provider (`aws_secrets_manager`, `azure_key_vault`, `hashicorp_vault`).
|No
...
|`managed`
|Pre-create mode: `true` for managed flow, `false` for reference flow. Defaults based on `raw_secrets` presence.
|No
...
.Example request (pre-create managed)
[source,json]
----
{
  "name": "test_integration_connection",
...
.Example response (fully connected)
[source,json]
----
{
...
.Example response (under-permissioned)
[source,json]
----
{
  "tool": "test_integration_connection",
