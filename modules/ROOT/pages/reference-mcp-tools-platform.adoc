...
Get token usage and response time timeseries for a single MCP server, agent, or model proxy instance. 
...
|`category`
|Scope of the query. Supported values: `mcp` (default) and `agent`. When set to `agent`, `instance_id` must be an agent ID (`agentId` or `llmProxyId` from `fetch_cost_instances`); buckets return token counts and `avgResponseTimeMs` (no per-bucket dollar figures).
|No

|`instance_id`
|Instance ID to query. For `mcp`, the MCP server instance ID. For agent, the `agentId` or `llmProxyId` returned by `fetch_cost_instances(category='agent')`.
|Yes
...
|`time_range`
|Time window for the timeseries. Supported values: `1h`, `6h`, `24h`, `7d`, `30d`.
|No
...
Get per-instance token usage and optimization coverage across MCP servers, agents, and model proxies. For `category='agent'`, dollar figures appear only when cost data is configured for the organization. Otherwise values are zeroed placeholders. To optimize an MCP instance, pass its `unappliedCostPolicyIds` to `prepare_policy_creation`.
...
|`category`
|Scope of the query. Supported values: `mcp` (default) and `agent`. When set to `agent`, returns per-agent dollar figures (`spend`, `saved`, `potentialSavings`) instead of token metrics; `filter_unoptimized` and `sort_by` do not apply.
|No
