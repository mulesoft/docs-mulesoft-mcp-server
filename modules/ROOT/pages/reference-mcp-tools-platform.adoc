...
Get instance-rooted lineage from the Context Catalog for an agent, MCP server, or API. Use this tool to for prompts such as "Show me everything upstream of the Fraud Detection MCP server" or 
"Trace the downstream lineage for the Billing Agent".
...
Get the source-to-column hierarchy, classifications, descriptions, glossary terms, and PII state for a data catalog source. Before using this tool, use find_assets with asset_type=data to discover available sources. Before using this tool, use `find_assets` with `asset_type=data` to discover available sources. Use this tool to answer questions such as "What's the PII classification for the customer_orders data source?" or "Is there any PII in the finance_reporting data catalog source?"
